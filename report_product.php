...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($reason)) {
        $error = "Please provide a reason for reporting this product.";
    } else {
        $stmt = $conn->prepare("INSERT INTO product_reports (product_id, reported_by, reason) VALUES (?, ?, ?)");
        $stmt->bind_param("iis", $product['id'], $_SESSION['user_id'], $reason);
        $stmt->execute();
        $stmt->close();

        // Redirect back to product page with success message
        header("Location: detail.php?id={$product['id']}&reported=success");
        exit();
    }
}
?>
...
<title>Report Product - EWU Add & Drop</title>
...
<section class="report-section">
    <div class="report-container">
        <h1>Report Product: "<?= htmlspecialchars($product['name']) ?>"</h1>
